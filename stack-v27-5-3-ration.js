...
function render(){if(innerWidth>720||!active())return;const body=document.querySelector('#v234Fitness .v234-body'),anchor=document.getElementById('v2752NutritionMinimal');if(!body||!anchor)return;document.getElementById('v2753Ration')?.remove();const v=variant(),plan=VARIANTS[v],done=plan.meals.filter((_x,i)=>isDone(v,i)).length,s=document.createElement('section');s.id='v2753Ration';s.className='v2753-card';s.innerHTML=`<div class="v2753-top"><div><div class="v2753-kicker">РАЦИОН · СЕГОДНЯ</div><h3>Предложенное питание</h3></div><span>${done}/${plan.meals.length}</span></div><div class="v2753-tabs">${VARIANTS.map((x,i)=>`<button class="${i===v?'on':''}" data-v2753-variant="${i}">${i+1}</button>`).join('')}</div><div class="v2753-list">${plan.meals.map((m,i)=>{const ok=isDone(v,i),a=allocation(i),items=foodItems(m[1],a);return `<article class="${ok?'done':''}"><div class="v2753-meal"><b>${m[0]}</b><p>${m[1]}</p>${a?`<small>≈ ${a.kcal} ккал</small>`:''}${items?`<ul class="v2753-items">${items.map(it=>it.grams?`<li><div class="v2753-item-row"><b>${it.name}</b><span>${it.grams} г</span></div><small>${it.kcal} ккал · Б${it.protein} Ж${it.fat} У${it.carbs}</small></li>`:`<li><div class="v2753-item-row"><b>${it.name}</b></div></li>`).join('')}</ul>`:''}</div><button data-v2753-mark="${i}">${ok?'✓ ПОЕЛ':'○ НЕ ЕЛ'}</button></article>`}).join('')}</div><div class="v2753-note">Порции подстрой под свой дневной ориентир. Это шаблон рациона, а не медицинское назначение.</div>`;anchor.after(s);s.querySelectorAll('[data-v2753-variant]').forEach(b=>b.onclick=()=>{localStorage.setItem(VARIANT,String(+b.dataset.v2753Variant));render()});s.querySelectorAll('[data-v2753-mark]').forEach(b=>b.onclick=()=>toggle(v,+b.dataset.v2753Mark));syncSummary()}
