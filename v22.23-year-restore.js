...
function patch(){let root=document.getElementById('v2220Finance'),tabs=root?.querySelector('.v2220-tabs'),yearBtn=tabs?.querySelector('[data-tab="year"].on');if(!root||!tabs||!yearBtn)return;let y=selectedYear(),incData=read(INCOME),arr=Array.from({length:12},(_,i)=>{let k=`${y}-${String(i+1).padStart(2,'0')}`;return{k,sv:savedAll(k),inc:Number(incData?.values?.[k])||0}}),inc=arr.reduce((s,z)=>s+z.inc,0),sv=arr.reduce((s,z)=>s+z.sv,0),mx=Math.max(1,...arr.map(z=>Math.abs(z.sv)));while(tabs.nextSibling)tabs.nextSibling.remove();tabs.insertAdjacentHTML('afterend',`<div class="v2220-title">${y} · все накопления в ₽</div><div class="v2220-grid"><div class="v2220-box"><span>ДОХОД</span><b>${fmt(inc)} ₽</b></div><div class="v2220-box"><span>ОТЛОЖЕНО · ВСЕ ВАЛЮТЫ</span><b class="v2220-green">${fmt(sv)} ₽</b></div></div>${arr.map((z,i)=>`<div class="v2220-mrow"><span>${MN[i].slice(0,3)}</span><div class="v2220-mbar"><i style="width:${Math.max(0,z.sv/mx*100)}%"></i></div><b>${fmt(z.sv)} ₽</b></div>`).join('')}`);root.dataset.yearRestored='1'}
function init(){let screen=document.getElementById('screenSavings');screen?.addEventListener('click',e=>{if(e.target.closest('#v2220Finance [data-tab="year"]'))setTimeout(patch,80)},true);console.info('STACK v22.23 annual analytics restore')}
