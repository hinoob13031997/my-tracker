/* STACK v22.42 SAFE BACKUP + serialized writes + data event
   Adds local backup history without changing the existing storage key or state format. */
(()=>{
'use strict';

const BUILD='v22.42-backup-serialized';
...
function toast(text,bad=false){
  ...
  clearTimeout(toast._t);toast._t=setTimeout(()=>{el.style.opacity='0';el.style.transform='translateY(-6px)'},1500);
}
async function loadBackups(){
  try{if(typeof idbOpen!=='function')return[];const db=await idbOpen();const value=await new Promise((resolve,reject)=>{const tx=db.transaction(IDB_STORE,'readonly'),req=tx.objectStore(IDB_STORE).get(BACKUP_KEY);req.onsuccess=()=>resolve(Array.isArray(req.result)?req.result:[]);req.onerror=()=>reject(req.error)});db.close();return value}catch(e){return[]}
}
async function writeBackup(snapshot){
  try{
    if(!snapshot||typeof snapshot!=='object'||typeof idbOpen!=='function')return;
    const list=await loadBackups(),payload=JSON.stringify(snapshot),latest=list[list.length-1];if(latest&&latest.payload===payload)return;
    list.push({ts:Date.now(),build:BUILD,payload});while(list.length>MAX_BACKUPS)list.shift();
    const db=await idbOpen();await new Promise((resolve,reject)=>{const tx=db.transaction(IDB_STORE,'readwrite');tx.objectStore(IDB_STORE).put(list,BACKUP_KEY);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close();
  }catch(e){}
}
function saveBackup(snapshot){
  let copy=null;try{copy=JSON.parse(JSON.stringify(snapshot))}catch(e){return backupQueue}
  backupQueue=backupQueue.then(()=>writeBackup(copy)).catch(()=>{});return backupQueue;
}
...
function wrapSave(){
  try{if(typeof save!=='function'||save.__stackBackupWrapped)return;const original=save;const wrapped=function(...args){backupPrevious();const result=original.apply(this,args);try{if(typeof state!=='undefined')saveBackup(state);window.dispatchEvent(new CustomEvent('stack:data-changed',{detail:{source:'core-save'}}));toast('✓ Сохранено')}catch(e){}return result};wrapped.__stackBackupWrapped=true;save=wrapped}catch(e){}
}
function persistBackup(){try{if(typeof state!=='undefined')saveBackup(state)}catch(e){}}
function boot(){installButton();wrapSave();persistBackup();window.addEventListener('pagehide',persistBackup);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')persistBackup()});console.info('STACK backup module',BUILD)}
