...
function route(screen,tab){
  ...
  setTimeout(()=>{
    ...
    if(screen==='screenSavings'&&tab){
      ...
      setTimeout(()=>document.querySelector(`#v2220Finance [data-tab="${tab}"]`)?.click(),50);
    }
  },30);
}
...
function handleClick(event){
  ...
  const finance=event.target.closest?.('[data-v24-finance]');if(finance){event.preventDefault();document.querySelector(`#v2220Finance [data-tab="${finance.dataset.v24Finance}"]`)?.click();return}
