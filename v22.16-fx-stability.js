/* STACK v22.33 — event-driven FX history repair on shared STACK_FX. No polling. */
(()=>{'use strict';
...
function currency(){
  ...
  let a=[...document.querySelectorAll('#screenSavings button')].find(x=>x.classList.contains('active')||x.getAttribute('aria-selected')==='true'),t=(a?.textContent||'').toUpperCase();
...
function renderFx(){
  ...
  if(cards[2]){
    cards[2].textContent=r?(x.diffRub>=0?'+':'')+fmt(x.diffRub)+' ₽':'—';
    cards[2].classList.toggle('v2214pos',x.diffRub>=0);
    cards[2].classList.toggle('v2214neg',x.diffRub<0)
  }
  let foot=box.querySelector('.v2214rate');
  if(foot)foot.innerHTML=`Всего: ${fmt(x.native,2)} ${c} · текущий курс ${r?fmt(r,2)+' ₽/'+c:'загрузка курса…'}${x.actual?` · <span style="color:#68d43f">фактический курс: ${x.actual} оп.</span>`:x.missing?` · <span style="color:#e0ad37">уточняем исторический курс: ${x.missing}</span>`:` · <span style="color:#68d43f">исторические курсы ЦБ: ${x.known}</span>`}`;
  box.dataset.v2233='1'
}
function signature(){
  return goals().map(g=>[g?.id,g?.currency,g?.start,g?.startDate,(D()?.transactions?.(g)||[]).map(t=>[t?.id,t?.date,t?.amount,t?.actualRate,t?.rubAmount]).join(':')].join('|')).join(';')+'|'+JSON.stringify(D()?.fxCache?.()||{})
}
function refresh(force=false){
  ...
  refreshTimer=setTimeout(()=>{
    renderFx();
    let sig=signature();
    if(force||sig!==last){last=sig;repairHistory()}
  },force?0:80)
}
...
function init(){installEvents();refresh(true);console.info('STACK v22.33 FX stability on shared engine')}
