...
function boot(){style();const root=document.querySelector('#v234Fitness #v234Nutrition');if(root){observer=new MutationObserver(()=>setTimeout(apply,0));observer.observe(root,{childList:true,subtree:true})}document.addEventListener('click',e=>{if(e.target.closest('[data-v234="nutrition"]'))queueMicrotask(apply)});window.addEventListener('stack:data-changed',()=>setTimeout(apply,20));window.addEventListener('resize',()=>setTimeout(apply,30));setTimeout(apply,40);Object.defineProperty(globalThis,'STACK_NUTRITION_SIMPLIFY',{value:Object.freeze({build:BUILD,refresh:apply}),configurable:true});console.info('STACK nutrition simplify',BUILD)}
