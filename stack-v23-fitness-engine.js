/* STACK v24.2 — goal-aware training with set-based exercise completion. */
(()=>{'use strict';
const BUILD='v24.2-fitness-set-completion';
...
function modal(id,date){
 ...
 const all=read(SETS_KEY,[]),last=all.filter(x=>x.exercise===id).sort((a,b)=>String(a.date).localeCompare(String(b.date))).pop(),todaySets=all.filter(x=>x.exercise===id&&x.date===date),nextSet=Math.max(0,...todaySets.map(x=>Number(x.set)||0))+1;
