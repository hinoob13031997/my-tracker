/* STACK v24.4 — goal-aware training with reversible set-based completion. */
(()=>{'use strict';
const BUILD='v24.4-fitness-set-management';
...
.fx-engine-ex.completed{margin:0 -5px;padding:10px 5px;border-color:#285a3d;background:linear-gradient(90deg,#14331f99,transparent);border-radius:10px}.fx-engine-ex.completed b{text-decoration:line-through;color:#83d99a}.fx-engine-ex.completed .fx-engine-num{border-color:#68d43f;background:#15351e;color:#8cf06e;box-shadow:0 0 9px #68d43f55}.fx-engine-ex.completed .fx-engine-log{border-color:#68d43f;background:#102c19;color:#8cf06e}.fx-engine-set-count{color:#a9b8ca}.fx-engine-ex.completed .fx-engine-set-count{color:#68d43f}
...
.fx-set-list{display:grid;gap:6px;margin:10px 0;padding:9px;border:1px solid #1a405b;border-radius:12px;background:#050e19}.fx-set-list-title{display:flex;justify-content:space-between;align-items:center;color:#92a1b5;font-size:8px;font-weight:900}.fx-set-row{display:grid;grid-template-columns:24px 1fr 44px;gap:8px;align-items:center;padding-top:6px;border-top:1px solid #132a3f}.fx-set-row b{font-size:10px}.fx-set-row small{display:block;margin-top:2px;color:#8090a5;font-size:8px}.fx-set-remove{width:44px;height:44px;border:1px solid #5a2938;border-radius:10px;background:#170a10;color:#f06a83;font-size:18px}.fx-reset-sets{width:100%;min-height:44px;border:1px solid #5a2938;border-radius:10px;background:#170a10;color:#f06a83;font-size:9px;font-weight:900}
}`;
...
function targetSets(id,wo){
 if(wo.light)return 2;
 const phase=wo.phase.name,direction=goalDirection();
 if(phase==='Рост объёма'&&direction==='gain'&&!['core','sideplank'].includes(id))return 4;
 if(phase==='Сила + масса'&&direction==='gain'&&['squat','bench','rdl'].includes(id))return 4;
 return 3;
}

function setProgress(id,date,wo){
 const rows=read(SETS_KEY,[]).filter(x=>x.exercise===id&&x.date===date&&Number(x.reps)>0);
 const completed=new Set(rows.map(x=>Number(x.set)||0).filter(Boolean)).size;
 const target=targetSets(id,wo);
 return{completed,target,done:completed>=target};
}
...
function todayCard(d,wo){
 ...
 return `<section class="fx-engine-card" data-engine-today><div class="fx-engine-head"><div><div class="fx-eye">FITNESS · ВЫБРАННЫЙ ДЕНЬ</div><h2>${wo.light?'Разгрузка · ':''}${wo.name}</h2><div class="fx-engine-note">${label} · ${wo.light?'35–45':'45–65'} минут</div></div><span class="fx-engine-week">НЕДЕЛЯ ${w}</span></div><div class="fx-engine-chips"><span class="hot">${phaseText(wo.phase).name}</span><span>ЦИКЛ ${wo.cycle}</span><span>${wo.light?'ОБЛЕГЧЁННАЯ НЕДЕЛЯ':'РАБОЧАЯ НЕДЕЛЯ'}</span></div>${wo.ids.map((id,i)=>{const e=EX[id],base=wo.baseIds[i],progress=setProgress(id,key(d),wo);return `<div class="fx-engine-ex ${progress.done?'completed':''}"><span class="fx-engine-num">${progress.done?'✓':i+1}</span><div><b>${e.name}</b><small>${prescription(id,wo)} · <span class="fx-engine-set-count">${progress.completed}/${progress.target} подхода</span></small></div><button class="fx-engine-icon" data-info="${id}" aria-label="Как выполнять ${esc(e.name)}">i</button><button class="fx-engine-icon fx-engine-swap" data-swap-exercise="${id}" data-swap-base="${base}" data-swap-block="${wo.cycle}" aria-label="Заменить ${esc(e.name)}">↻</button><button class="fx-engine-icon fx-engine-log" data-log-exercise="${id}" data-log-date="${key(d)}" aria-label="Записать подходы ${esc(e.name)}">${progress.done?'✓':'＋'}</button></div>`}).join('')}<div class="fx-help">${suggestion(wo.ids[0])}</div><div class="fx-engine-status"><button class="done ${st==='done'?'on':''}" data-engine-status="done">✓ ВЫПОЛНЕНА</button><button class="skip ${st==='skip'?'on':''}" data-engine-status="skip">○ ПРОПУЩЕНА</button><button class="clear ${!st?'on':''}" data-engine-status="">· БЕЗ ОТМЕТКИ</button></div></section>`;
}
...
function modal(id,date){
 ...
 const all=read(SETS_KEY,[]),last=all.filter(x=>x.exercise===id).sort((a,b)=>String(a.date).localeCompare(String(b.date))).pop(),todayEntries=all.map((row,index)=>({row,index})).filter(x=>x.row.exercise===id&&x.row.date===date),todaySets=todayEntries.map(x=>x.row),nextSet=Math.max(0,...todaySets.map(x=>Number(x.set)||0))+1;
 const history=todayEntries.length?`<div class="fx-set-list"><div class="fx-set-list-title"><span>ПОДХОДЫ ЗА ЭТОТ ДЕНЬ</span><span>${todayEntries.length}</span></div>${todayEntries.map(x=>`<div class="fx-set-row"><b>${Number(x.row.set)||'—'}</b><div><strong>${Number(x.row.weight)||0} кг × ${Number(x.row.reps)||0}</strong><small>${x.row.effort==='easy'?'Легко':x.row.effort==='hard'?'Тяжело':x.row.effort==='limit'?'Предел':'Нормально'}</small></div><button type="button" class="fx-set-remove" data-set-remove="${x.index}" aria-label="Удалить подход ${Number(x.row.set)||''}">×</button></div>`).join('')}<button type="button" class="fx-reset-sets" data-reset-sets>СБРОСИТЬ ВСЕ ПОДХОДЫ УПРАЖНЕНИЯ</button></div>`:''; 
 const measure=e.unit==='сек'?'Время, секунд':'Повторения';const s=document.createElement('div');s.id='fxEngineModal';s.className='fx-sheet';s.innerHTML=`<div class="fx-editor"><div class="fx-eye">ЗАПИСЬ ПОДХОДА</div><h3>${e.name}</h3>${history}<form data-engine-set><input type="hidden" name="exercise" value="${id}"><input type="hidden" name="date" value="${date}"><div class="fx-form-grid"><label>Номер подхода<input name="set" type="number" inputmode="numeric" min="1" max="12" value="${nextSet}" required></label><label>Рабочий вес, кг<input name="weight" type="number" inputmode="decimal" min="0" step="0.1" value="${last?.weight??''}" placeholder="0"></label></div><label>${measure}<input name="reps" type="number" inputmode="numeric" min="1" max="300" value="${last?.reps??''}" required></label><label>Насколько тяжело было?</label><div class="fx-effort"><label><input type="radio" name="effort" value="easy">Легко<br>ещё 3–4</label><label><input type="radio" name="effort" value="normal" checked>Нормально<br>ещё около 2</label><label><input type="radio" name="effort" value="hard">Тяжело<br>ещё примерно 1</label><label><input type="radio" name="effort" value="limit">Предел<br>запаса нет</label></div><div class="fx-help">Оцени запас: сколько повторов ты ещё смог бы сделать с правильной техникой.</div><button class="fx-save">СОХРАНИТЬ ПОДХОД</button><button type="button" class="fx-close" data-engine-close>ЗАКРЫТЬ</button></form></div>`;document.body.appendChild(s);
 s.onclick=x=>{if(x.target===s||x.target.closest('[data-engine-close]'))s.remove()};
 s.querySelectorAll('[data-set-remove]').forEach(button=>button.onclick=()=>{const a=read(SETS_KEY,[]),index=Number(button.dataset.setRemove);if(a[index]?.exercise===id&&a[index]?.date===date)a.splice(index,1);write(SETS_KEY,a);window.dispatchEvent(new CustomEvent('stack:data-changed',{detail:{source:'fitness-set-remove'}}));s.remove();refresh();setTimeout(()=>modal(id,date),60)});
 const reset=s.querySelector('[data-reset-sets]');if(reset)reset.onclick=()=>{if(!confirm('Удалить все подходы этого упражнения за выбранный день?'))return;const a=read(SETS_KEY,[]).filter(x=>x.exercise!==id||x.date!==date);write(SETS_KEY,a);window.dispatchEvent(new CustomEvent('stack:data-changed',{detail:{source:'fitness-sets-reset'}}));s.remove();refresh()};
 s.querySelector('[data-engine-set]').onsubmit=x=>{x.preventDefault();const f=new FormData(x.currentTarget),a=read(SETS_KEY,[]),row={id:Date.now().toString(36),date:String(f.get('date')),exercise:String(f.get('exercise')),set:+f.get('set'),weight:+f.get('weight')||0,reps:+f.get('reps'),effort:String(f.get('effort'))},existing=a.findIndex(v=>v.date===row.date&&v.exercise===row.exercise&&Number(v.set)===row.set);if(existing>=0){row.id=a[existing].id||row.id;a[existing]=row}else a.push(row);write(SETS_KEY,a);window.dispatchEvent(new CustomEvent('stack:data-changed',{detail:{source:'fitness-sets'}}));s.remove();refresh()};
}
