...
async function boot(){shell();
  if(!V29())await load('stackV28MinimalScript',`./stack-v28-minimal.js?build=${BUILD}`,()=>!!globalThis.STACK_MINIMAL);
  ...
  if(!V29())globalThis.STACK_MINIMAL?.refresh?.();globalThis.STACK_SECTIONS?.refresh?.();shell();document.documentElement.classList.remove('stack-minimal-loading');
  window.addEventListener('pageshow',()=>{shell();if(!V29())globalThis.STACK_MINIMAL?.refresh?.()});document.addEventListener('visibilitychange',()=>{if(!document.hidden){shell();if(!V29())globalThis.STACK_MINIMAL?.refresh?.()}});Object.defineProperty(globalThis,'STACK_BOOTSTRAP',{value:Object.freeze({build:BUILD,refresh:shell}),configurable:true});console.info('STACK bootstrap',BUILD)}
preflight();
